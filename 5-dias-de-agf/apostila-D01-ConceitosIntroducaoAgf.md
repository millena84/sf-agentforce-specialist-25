...
3. Conceitos de IA
4. Porque Agentforce
5. Visão geral arquitetura Agentforce / funcionamento
...
## D01: IA, Agentes, Conceitos de IA e introdução ao Agentforce
...
##### Perguntinhas mágicas (checklist)
...
Em geral, orquestrar um agente de IA (modelo, chamadas, monitoramento) tende a ser mais caro e complexo do que uma automação simples ou um CRED/CRUD bem feito. Por isso, ele precisa entregar valor em problemas onde automação pura não chega bem.
...
- ***Limites de tokens por LLM:***
   ...
   - Sem data masking, cada modelo tem o seu limite próprio limite:
      - Maiores detalhes, acesse [LLM limits - Help salesforce](https://help.salesforce.com/s/articleView?id=ai.generative_ai_llm_limits.htm&type=5)
...
Esses são alguns conceitos que do meu humilde ponto de vista é interessante que os alunos saibam que existe.
>Não usaremos esse conhecimento neste treinamento (EXCETO OS DETALHES RELACIONADOS A CHUNK!)

###### Relacionados a memória, contexto

Existem outras técnicas de se fazer um modelo LLM usar base de conhecimento ou aprender/ser treinado:
...
>💬 "Oh, mas porque você acha importante conhecer isso?"
Porque numa discussão técnica, podem ser mencionadas outras estratégias de enriquecimento de dados. O que abordamos aqui é o RAG porque ele é massivamente usado no Agentforce, mas é essencial saber que existem outras maneiras.
...
- **Estado de busca**: 
   ...
   - Em RAG:
      - podemos pensar como “o que o sistema sabe agora sobre a pergunta?”. Ou seja, as informações que ele tem **antes da execução**: pergunta do usuário + contexto da ação/conversa (aqui imaginando o início de uma iteração).
- **Espaço de busca**:
   ...
   - Em RAG: Todos os chunks (pedaços) da base de conhecimento disponível no momento de execução do LLM **que serão considerados para a busca (no momento do retrieval)**.
...
###### Relacionados a Chunking

Para melhor aproveitar as capacidades de RAG e fazer configurações mais eficientes, é importante conhecer outros pontos relacionados a chunking.

- **Técnicas comuns de Chunking:**
...
- **Influência sobre o RAG:**
   - **Quando ajuda:**
      ...
      - mas pequenos o bastante para serem bem ranqueados e caberem no contexto com vários exemplos relevantes
         - Estado de busca efetivo é reduzido à unidades semânticas úteis
...
- Construção por low-code (sem código direto): Agentforce Agents / Agentforce Builder traz um fluxo unificado para detalhamento e integração de automações;
...
- Atlas reasoning engine, o cérebro do Agente (falaremos dele em instantes): Core, o núcleo central do Agentforce, motor de raciocínio que segue um ciclo iterativo de reason-act-learn (pensar-agir-aprender).
   - Reason (Pensar): Processa as entradas recebidas (texto, eventos, dados), identifica os temas envolvidos e elabora um plano de ação combinando um LLM com as instruções definidas.
   - Act (Agir): Executa as ações necessárias — como acionar Flows, Apex, APIs ou ferramentas externas — por meio de chamadas de função estruturadas.
   - Learn (Aprender): Avalia os resultados obtidos, valida se estão fundamentados (grounding) e repete o ciclo quantas vezes forem necessárias até atingir o objetivo — ou, se não conseguir, escala a situação para um humano.
- Einstein Trust Layer fazendo a camada de segurança - também falaremos adiante.

---

### Visão geral Agentforce
...
<!-- https://github.com/millena84/sf-agentforce-specialist-25/tree/main/5-dias-de-agf -->

<center>
<img src="./img/d01/D01-arquit-base-Agentforce360Platform.png" width="70%" alt="Pilares Agentforce 360 Platform">
...
![Interação Agentforce](./img/d01/d01-interacao-agentforce.jpg)
...
![Einstein Trust Layer](https://res.cloudinary.com/hy4kyit2a/f_auto,fl_lossy,q_70/learn/modules/the-einstein-trust-layer/meet-the-einstein-trust-layer/images/pt-BR/b7e7e25de679f0a6b881906d59a9cf00_kix.u0bgnhwtkwm3.png)
...
#### Lembra dos conceitos? (ponte)

<center>

<!-- https://github.com/millena84/sf-agentforce-specialist-25/tree/main/5-dias-de-agf -->

<img src="./img/d01/d01-interacao-agentforce-e-conceitos-ia.jpg" width="80%" alt="Ecossistema Agentforce e Conceitos de IA">
...
- **Vectorize**: Armazenar os embeddings (vetores) em um índice vetorial que permite buscas rápidas por similaridade
- **Chunking optimization**: Como você quer configurar o chunk da sua base de conhecimento
...
- **Multi-representation indexing**: Quando criado um search index, são criadas automaticamente 2 tabelas DMOS (tabela no Data 360/Cloud).
      - Chunk DMO: dado
      - Summary DMO: embedding
- **Specialized embbedings**: É possível no data cloud escolher entre modelos diferentes de embedding.
- **Hierarquical indexing**: É um método avançado de organização de dados para busca e recuperação mais eficiente em sistemas RAG
